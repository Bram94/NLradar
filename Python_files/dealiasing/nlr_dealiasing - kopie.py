...
class DualPRFDealiasing():
    ...
    def detect_outliers(self):
        ...
        t = pytime.time()
        sin_phi_mean, cos_phi_mean = self.calculate_sin_cos_phi_mean(self.window_detection)        
        print(pytime.time()-t, 't_tikkie')
        phi_ref = np.arctan2(sin_phi_mean[self.data_nonmask],cos_phi_mean[self.data_nonmask])
    ...
    def correct_outliers(self):
        ...
        sin_phi_mean, cos_phi_mean = self.calculate_sin_cos_phi_mean(self.window_correction)        
        phi_ref = np.arctan2(sin_phi_mean[self.outliers], cos_phi_mean[self.outliers])
    ...
    def calculate_sin_cos_phi_mean(self, window):
        sin_phi_mean = np.zeros((self.n_azi, self.n_rad)); cos_phi_mean = sin_phi_mean.copy()
        sin_phi_mean, cos_phi_mean = get_window_sum(self.sin_phi, window), get_window_sum(self.cos_phi, window)
        return sin_phi_mean, cos_phi_mean
