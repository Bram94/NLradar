...
class DualPRFDealiasing():
    ...
    window_detection = [2, 2, 2, 2, 2], window_correction = [2, 2, 2, 2, 2], deviation_factor = 1.0, n_it = 'test', c_array = None, mask_nearzero_velocities = False, min_gates_correct = 0, plot_velocity_deviations = False):
        t = pytime.time()
        """Parameters:
        ...
        self.data = v_array
        self.data_indices = np.array(np.meshgrid(np.arange(self.data.shape[0]), np.arange(self.data.shape[1]))).T
    ...
    def dealiase(self):
        ...
        self.start = True
        self.n_outliers_before = 0; self.n_outliers = None
        n_it = 0
        while self.n_outliers != self.n_outliers_before and n_it < self.n_it:
            if self.start:
    ...
    def detect_outliers(self):
        ...
        if not hasattr(self, 'sin_phi_sum_detection'):
            self.sin_phi_sum_detection, self.cos_phi_sum_detection = self.calculate_sin_cos_phi_sum(self.window_detection)        
        phi_ref = np.arctan2(self.sin_phi_sum_detection[self.data_nonmask], self.cos_phi_sum_detection[self.data_nonmask])
        ...
        print(self.n_outliers_before, self.n_outliers)
        if self.n_outliers == self.n_outliers_before:
            return
        
        if not self.start:
            rows, cols = get_window_indices(self.data_indices[self.outliers], self.window_correction, self.data.shape, periodic = 'rows')
            window_size = sum([j*2+1 for j in self.window_correction])
            self.sin_phi_sum_correction[(rows, cols)] += np.tile(0 - self.sin_phi[self.outliers], window_size)
            self.cos_phi_sum_correction[(rows, cols)] += np.tile(0 - self.cos_phi[self.outliers], window_size)
    ...
    def correct_outliers(self):
        ...
        avg_a is minimised."""
        old_sin_phi_outliers, old_cos_phi_outliers = self.sin_phi[self.outliers], self.cos_phi[self.outliers]
        ...
        if not hasattr(self, 'sin_phi_sum_correction'):
            self.sin_phi_sum_correction, self.cos_phi_sum_correction = self.calculate_sin_cos_phi_sum(self.window_correction)     
        phi_ref = np.arctan2(self.sin_phi_sum_correction[self.outliers], self.cos_phi_sum_correction[self.outliers])
        ...
        t = pytime.time()
        rows, cols = get_window_indices(self.data_indices[self.outliers], self.window_detection, self.data.shape, periodic = 'rows')
        window_size = sum([j*2+1 for j in self.window_detection])
        self.sin_phi_sum_detection[(rows, cols)] += np.tile(self.sin_phi[self.outliers] - old_sin_phi_outliers, window_size)
        self.cos_phi_sum_detection[(rows, cols)] += np.tile(self.cos_phi[self.outliers] - old_cos_phi_outliers, window_size)
        
        rows, cols = get_window_indices(self.data_indices[self.outliers], self.window_correction, self.data.shape, periodic = 'rows')
        window_size = sum([j*2+1 for j in self.window_correction])
        self.sin_phi_sum_correction[(rows, cols)] += np.tile(self.sin_phi[self.outliers], window_size)
        self.cos_phi_sum_correction[(rows, cols)] += np.tile(self.cos_phi[self.outliers], window_size)

        

        print(pytime.time() - t, 't_test')
        
        
    def calculate_sin_cos_phi_sum(self, window):
        sin_phi_sum = np.zeros((self.n_azi, self.n_rad)); cos_phi_sum = sin_phi_sum.copy()
        sin_phi_sum, cos_phi_sum = get_window_sum(self.sin_phi, window), get_window_sum(self.cos_phi, window)
        return sin_phi_sum, cos_phi_sum
...
def apply_dual_prf_dealiasing(v_array, data_mask, azimuthal_res, radial_res, vn_e, vn_l, vn_h, vn_first_azimuth = None, window_detection = None, window_correction = None, deviation_factor = 1.0, n_it = 'test', c_array = None, mask_nearzero_velocities = False):
