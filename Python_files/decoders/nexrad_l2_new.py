...
class NEXRADLevel2File:
    ...
    def decode_file(self, file_name_or_obj, read_mode="all"):
        ...
        if read_mode.endswith('meta'):
            if self._bzip2_compression:
                # Make sure that for bzip2 compression also the 1st compressed message gets included below
                self._fh.seek(0)
                cbuf = self._fh.read()
                self._bzip2_start_pos = _get_bzip2_start_indices(cbuf)
                bzip2_read_indices = 'all'
                if read_mode == 'min-meta':
                    buf = _decompress_records_meta(cbuf, self._bzip2_start_pos, [0])
                    self._read_records(buf)
                    self._get_vcp()
                    if self.vcp:
                        cut_params = self.vcp['cut_parameters']
                        scans_indices = [0]
                        expected_last_bzip2_index = 0
                        for scan in cut_params:
                            expected_last_bzip2_index += 3+3*(scan['super_resolution'] in (11, 7))
                            scans_indices.append(expected_last_bzip2_index)
                        if scans_indices[-1]+1 == len(self._bzip2_start_pos):
                            bzip2_read_indices = scans_indices
                buf = _decompress_records_meta(cbuf, self._bzip2_start_pos, bzip2_read_indices)
        ...
        for i, r in enumerate(self._records):
            if r["header"]["type"] in (1, 31):
                ...
                if read_mode.endswith('meta') and self._bzip2_compression:
        ...
        if read_mode.endswith('meta'):
            ...
            for i, msgs in enumerate(self.scan_msgs):
                ...
                if self._bzip2_compression:
                    if read_mode == 'min-meta':
                        start_pos = self._bzip2_start_pos[pos_m1+1] if pos != pos_m1 else self._bzip2_start_pos[1]
                        end_pos = self._bzip2_start_pos[pos+1] if pos != pos_p1 else None
                    else:
                        start_pos = self._bzip2_start_pos[pos]
                        end_pos = self._bzip2_start_pos[pos_p1] if pos != pos_p1 else None
                else:
                    start_pos = pos
                    end_pos = pos_p1 if pos != pos_p1 else None
    ...
    def _read_records(self, buf):
        ...
        for i,b in enumerate(buf):
            ...
            while pos < buf_length:
                ...
                if self._bzip2_compression and self._read_mode.endswith('meta') and i > 0 and not 'RAD' in dic:
                    b = _decompress_records_meta(cbuf, self._bzip2_start_pos, bzip2_read_indices=[i], max_length=10000)[0]
                    buf_length = len(b)
                    pos = 0
                    while pos < buf_length:
                        pos, dic = _get_record_from_buf(b, pos)
                        if 'RAD' in dic:
                            break
                elif self._bzip2_compression and self._read_mode.endswith('meta') and i == 0 and not dic["header"]["type"] in (1, 31, 5):
                ...
                if self._bzip2_compression and self._read_mode.endswith('meta'):
