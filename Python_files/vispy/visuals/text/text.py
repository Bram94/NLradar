...
class TextVisual(Visual):
    ...
    def __init__(self, text=None, color='black', bold=False,
        ...
        self._previous_text_scale = np.zeros(2)
    ...
    def _prepare_draw(self, view):
        ...
        n_pix = (self._font_size / 72.) * self.transforms.dpi  # logical pix
        tr = self.transforms.get_transform('document', 'render')
        px_scale = (tr.map((1, 0)) - tr.map((0, 1)))[:2]
        text_scale = n_pix * px_scale
        if np.any(text_scale != self._previous_text_scale):
            self._text_scale.scale = text_scale
            self.shared_program.vert['text_scale'] = self._text_scale
            self.shared_program['u_npix'] = n_pix
            self._previous_text_scale = text_scale
